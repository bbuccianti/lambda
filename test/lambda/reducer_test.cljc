(ns lambda.reducer-test
  (:require
   ...
   [lambda.reducer :refer [all-reductions can-reduce? step]]
   [lambda.stringy :refer [toString]]))

(deftest restoring
  (are [exp act] (= exp (-> act lex restore parse toString))
    ""
...
(deftest alpha-rule-works
  (are [exp act] (= exp (-> act lex restore parse step toString))
    ;; "(λx_1.(λy.(λx.x y z))) a b c" "(λx y x.x y z) a b c"

    ;; "(λx y z.x z (y z)) ((λx y.x) (λx y z.x z (y z))) (λx y.x)"
    ;; "(λy_1 z .((λx y.x) (λx y z.x z (y z))) z (y z)) (λx y.x)"
